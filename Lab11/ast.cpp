    ...
    stmt(s) 
{
    ...
    before = NewLabel(); // Label for the start of the loop
    after = NewLabel();  // Label for after the loop
}

void While::Gen()
{
    cout << 'L' << before << ':' << endl; // Print the start label
    Expression * n = Rvalue(expr); // Evaluate the loop condition
    cout << "\tifFalse " << n->ToString() << " goto L" << after << endl; // Check the condition
    stmt->Gen(); // Generate code for the loop body
    cout << "\tgoto L" << before << endl; // Go back to the start of the loop
    cout << 'L' << after << ':' << endl; // Print the exit label
}
